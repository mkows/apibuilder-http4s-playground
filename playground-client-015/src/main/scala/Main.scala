...
object Main extends App {
  ...
  println("Running (for http4s ver. 0.15)")
  val id = 1
  ...
  def go(id: Int): Unit = {
    ...
    res.handle {
      case errorResponse: ErrorResponse =>
        val details = s"[response=${errorResponse.response}, message=${errorResponse.message}, error=${errorResponse.error.run}]"
      ...
      case valueResponse : ValueResponse =>
        val details = s"[response=${valueResponse.response}, message=${valueResponse.message}, value=${valueResponse.value.run}]"
